...
# Google AI Studio 플랫폼 사이트에서 키 발급

# 라이브러리 설치
# pip install -q -U google-genai
# 0. api key는 노출되면 안됨. 그래서 별도의 환경변수 파일 .env에 저장하여 dotenv모듈로 불러서 적용
# 단, 우리의 챗봇을 streamlit cloud에 배포할 예정. 애석하게 이곳은 .env를 설정할 수 없음.
# 그래서 streamlit에서 제공하는 비밀 값을 저장하는 속성 secrets를 활용 
# [secrets에 등록될 값들은 .streamlit폴더 안에 secrets.toml 파일로 등록. 이 파일을 GitHub를 통해 배포되면 안됨. 노출되면 일정시간 후에 정지됨. 다시 키발급필요]
...
# 응답 제어를 위한 하이퍼 파라미터 설정
from google.genai import types 
config= types.GenerateContentConfig(
    max_output_tokens=10000,
    response_mime_type='text/plain',
    #system_instruction='넌 만물박사야. 넌 최대 100글자안에서 어린이도 이해할수 있게 뭐든 설명해'
    #system_instruction='넌 모든 대답을 개조식으로 해.'
    system_instruction='넌 불량 고등학생이야. 비속어를 적당히 사용하고 뭐든 100글자 안에 말해.',
    #응답할 때, 특정 기능함수를 참고하여 답변...
    tools=[get_today], 
)

# '질문'을 파라미터로 받아서 GET AI로 응답한 글씨를 리턴해주는 기능함수 만들기
def get_ai_response(question):
    response= client._models.generate_content(
        model="gemini-3-flash-preview", 
        # model="gemini-2.5-flash"
        contents=question,
        # 모델의 응답방법을 설정하기 - 하이퍼파라미터 설정
        config= config
    )
...
#c. 사용자 채팅메세지를 입력받아 session_state에 저장하고 UI갱신
...
if question:
    ...
    #응답 - AI 응답요구기능 함수 호출 ... 응답대기 시간동안 보여줄 스피너 프로그래스 바 요소가 필요
